...
Normally you would need to create json files manually: one for the model and another to specify when to display it. With Oraxen, you just specify the textures for your item. Oraxen handles all the resource pack generation automatically—see [Item Appearance](/configuration/item-appearance) for full details.
