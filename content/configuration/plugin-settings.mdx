...
The `appearance.mode` setting controls how item textures are applied on 1.21.4+:

| Mode | Pack Output | Item Component | Description |
|------|-------------|----------------|-------------|
| `ITEM_PROPERTIES` | `assets/oraxen/items/<id>.json` | `item_model` component | **Default.** Uses item model definitions with Oraxen namespaced ids (`oraxen:<item_id>`). |
...
| `MODEL_DATA_FLOAT_LEGACY` | `assets/minecraft/items/<material>.json` | `custom_model_data.floats[0]` | Uses `minecraft:range_dispatch` dispatcher with numeric thresholds. Legacy compatibility mode. |

**Version behavior:**
- **Pre-1.21.4**: Always uses legacy predicate overrides (regardless of mode setting)
- **1.21.4+**: Uses the selected mode

#### Force Predicates

The `force_predicates` option generates legacy predicate overrides (`assets/minecraft/models/item/*.json`) **in addition to** the selected mode. This is useful for compatibility with external tools or plugins that still inspect legacy JSON overrides or rely on integer `custom_model_data`.

```yaml
appearance:
  mode: ITEM_PROPERTIES
  force_predicates: true  # Also generate legacy predicates
```

See [Item Appearance](/configuration/item-appearance#appearance-mode-1214) for detailed documentation and per-item options.
...
**Grid Glyphs Warning:** If you enable `disable_automatic_glyph_code`, you **must** manually define the `chars` list for any [grid-based glyphs](/configuration/glyphs#multi-character-grid-glyphs). Without manual character assignment, grid glyphs will receive new unicode characters on every server restart, breaking saved references. Single-character glyphs will reuse their existing `char` value, but grid glyphs have no such fallback.
</Callout>
...
Glyphs:
  ...
  emoji_list_permission_only: false # Only show emojis player has permission for
