...
<Callout type="warning">
**Grid Glyphs Warning:** If you enable `disable_automatic_glyph_code`, you **must** manually define the `chars` list for any [grid-based glyphs](/configuration/glyphs#grid-based-glyphs). Without manual character assignment, grid glyphs will receive new unicode characters on every server restart, breaking saved references. Single-character glyphs will reuse their existing `char` value, but grid glyphs have no such fallback.
</Callout>
