...
```yaml
  generation:
    ...
    appearance:
      item_model: true    # Generate item model definitions (1.21.4+ only)
      predicates: false   # Generate predicate overrides (custom_model_data)
```
...
The `appearance` section controls how item textures are applied:

| Setting | Description |
|---------|-------------|
| `item_model` | Generate Item Model Definitions in `assets/oraxen/items/`. Items use the `item_model` component. |
| `predicates` | Generate predicate overrides in vanilla item models. Items use `custom_model_data`. |

**Default behavior by version:**
- **Pre-1.21.4**: Only predicates (forced, regardless of settings)
- **1.21.4+**: Only item_model by default

**Common configurations:**
- `item_model: true, predicates: false` - Modern (default): Clean 1.21.4+ setup
- `item_model: true, predicates: true` - Hybrid: Both systems for max compatibility
- `item_model: false, predicates: true` - Legacy: Classic custom_model_data behavior

See [Item Appearance](/configuration/item-appearance#appearance-system-settings-1214) for detailed documentation and per-item options.
