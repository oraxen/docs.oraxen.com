...
| `model_data_float_legacy` | `assets/minecraft/items/<material>.json` | `custom_model_data.floats[0]` | Uses `minecraft:range_dispatch` with numeric thresholds. Also generates legacy predicate overrides. |
...
- `model_data_float_legacy: true` — Full legacy compatibility (auto-generates predicates)

<Callout type="warning">
`model_data_ids` and `model_data_float_legacy` cannot both be enabled—they write to the same pack file path. If both are enabled, `model_data_ids` takes priority.
</Callout>

See [Item Appearance](/configuration/item-appearance#appearance-systems-1214) for detailed documentation and per-item options.
