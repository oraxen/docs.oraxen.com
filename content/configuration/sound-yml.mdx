...
- **Place your .ogg file** in `plugins/Oraxen/pack/sounds/` (create subdirectories if needed)

- **Register in sound.yml:**
...
- **Reload Oraxen:** `/oraxen reload all`

- **Use in game:** Play with `/playsound oraxen:my_custom_sound`
...
- **Check file format:** Must be `.ogg` (OGG Vorbis)
- **Verify file path:** Check `plugins/Oraxen/pack/sounds/<path>`
- **Reload Oraxen:** `/oraxen reload all`
- **Resend pack:** `/oraxen pack send @a`
- **Check client logs:** F3 + T to reload resource pack
...
- Verify `song_key` matches sound ID: `oraxen:<sound_id>`
- Ensure `stream: true` is set
- Check `length_in_seconds` is accurate
- Confirm item has `jukebox_playable` component
