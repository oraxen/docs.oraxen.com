...
Oraxen supports two systems for item appearance, which can be used independently or together:

| System | Pack Output | Item Component | Versions |
|--------|-------------|----------------|----------|
| **Item Model Definitions** | `assets/oraxen/items/<id>.json` | `item_model` component | 1.21.4+ only |
| **Predicates** | `assets/minecraft/models/item/<material>.json` | `custom_model_data` | All versions |

<Callout type="info">
**Default behavior:**
- **Pre-1.21.4**: Only predicates (forced)
- **1.21.4+**: Only item_model (default, configurable)
</Callout>

### Appearance System Settings (1.21.4+)

On 1.21.4+, you can configure which systems to use in `settings.yml`:

```yaml
Pack:
  generation:
    appearance:
      item_model: true    # Generate item model definitions
      predicates: false   # Generate predicate overrides
```

**Available modes:**

| Mode | item_model | predicates | Use Case |
|------|------------|------------|----------|
| **Modern** (default) | `true` | `false` | Clean 1.21.4+ setup |
| **Hybrid** | `true` | `true` | Compatibility with plugins using CMD |
| **Legacy** | `false` | `true` | Full backwards compatibility |

<Callout type="warning">
**Hybrid mode** generates both systems, making your items work with native 1.21.4+ features AND plugins that rely on custom_model_data. This increases pack size slightly but maximizes compatibility.
</Callout>
...
On 1.21.4+, this value is used when the predicates system is enabled (see [Appearance System Settings](#appearance-system-settings-1214) above). If you're using hybrid or legacy mode, this lets you control which CMD value your item uses for compatibility with other plugins.
