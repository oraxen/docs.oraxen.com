...
## Multi-Character Grid Glyphs

For large textures or sprite sheets, you can use grid-based glyphs that span multiple unicode characters. This is useful for:
- Large UI elements that exceed Minecraft's single-character size limit
- Sprite sheets containing multiple related images
- Progressive animations or multi-part displays

### Grid Configuration

```yaml
large_banner:
  texture: ui/large_banner
  ascent: 8
  height: 128
  grid:
    rows: 2
...
This creates a 2×4 grid (8 total characters). Oraxen automatically assigns unicode characters to each cell and saves them to your config as a `chars` list.

### Manual Character Assignment

If you need precise control over which unicode characters are used, you can define them manually:

```yaml
sprite_sheet:
  texture: ui/sprite_sheet
  ascent: 8
  height: 64
  chars:
    - "䀀䀁䀂䀃"  # Row 1: 4 characters
    - "䀄䀅䀆䀇"  # Row 2: 4 characters
  grid:
    rows: 2
    columns: 4
```
...
**Important:** If you have `ConfigsTools.disable_automatic_glyph_code: true` in your settings, you **must** manually define the `chars` list for grid glyphs. Otherwise, characters will be regenerated on every server restart, breaking any saved references to those characters.
</Callout>

### Using Grid Glyphs

Grid glyphs can be used with index or range selectors in the `<glyph>` tag:

```
<glyph:large_banner>         <!-- All characters with newlines between rows -->
<glyph:large_banner:1>       <!-- First character only -->
<glyph:large_banner:3>       <!-- Third character -->
<glyph:large_banner:1..4>    <!-- Characters 1-4 (first row) -->
```

Indices are 1-based and count left-to-right, top-to-bottom across the grid.

## Appearance Configuration

You can customize how glyphs are rendered with the `appearance` section:

```yaml
styled_icon:
  texture: icons/special
  ascent: 8
  height: 8
  appearance:
    font: "minecraft:default"     # Which font to use (default: minecraft:default)
    shadow_color: "#80000000"     # Shadow color in ARGB hex (1.21.4+ only)
```

### Shadow Colors

Shadow colors use ARGB hex format:
- `#AARRGGBB` - Full format with alpha
- `#RRGGBB` - Assumes full opacity (FF alpha)
- `#RGB` - Shorthand, expands to RRGGBB

Examples:
- `#80000000` - 50% transparent black shadow
- `#FF0000` - Red shadow (full opacity)
- `#00000000` - No shadow (fully transparent)

<Callout type="info">
Shadow colors require Minecraft 1.21.4 or newer. On older versions, this setting is ignored.
</Callout>

### Overriding Shadow in Tags

You can override the default shadow color per-use:

```
<glyph:my_icon:shadow:#FF0000>    <!-- Red shadow -->
<glyph:my_icon:s:#80FFFFFF>       <!-- 50% white shadow (short form) -->
<glyph:my_icon:colorable:shadow:#000000>  <!-- Colorable with black shadow -->
```
